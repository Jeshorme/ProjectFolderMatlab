function myGUI()
    ...
    passwordLabel = uicontrol('Style', 'text', 'String', '语音处理系统登录', 'Position', [50, 150, 250, 40],'fontsize',20);
    % 创建用户名文本框和标签
    ...
    function registerCallback(~, ~)
        ...
        % 将用户名和密码保存到文本文件中
        fileID = fopen('user_data.txt', 'a');
        fprintf(fileID, '用户名: %s, 密码: %s\n', username, password);
        fclose(fileID);
    ...
    function loginCallback(~, ~)
        ...
        % 从文件中读取保存的用户名和密码
        fileID = fopen('user_data.txt', 'r');
        userCredentials = textscan(fileID, '用户名: %[^,], 密码: %[^\n]');
        fclose(fileID);
        
        usernames = userCredentials{1};
        passwords = userCredentials{2};

...
if ~isempty(handles.Result)
    ...
    % 调用训练好的CNN模型进行识别裂缝类别
    net = load('model-database_judge');
    I = imresize(handles.Result.Image,[300 400]);
    label = classify(net.net,handles.Result.Image);
    label = string(label);
    handles.Result.str=label;
    tip = strcat('裂缝类型为：',label);
    msgbox(tip, '类型')
end
...
if ~isempty(handles.Result)
    ...
    %计算路面状况系数
    A = bwarea(handles.Result.Image);
    PID = 100-10.666*(handles.Result.BwArea/A)^0.461;
    %给定危险阈值
    dger=99;
    if PID<=dger
        msgbox('路面危险','路面情况')
...
try
    if ~isempty(handles.File)
        raw = [];
        xlsfile = fullfile(pwd, 'result.xls');
